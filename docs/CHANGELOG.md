...
- [文档] 新增数据需求与采集规范（`redesign/features/data-source/数据需求与采集规范.md`）：盘点 42 张表与采集链路，收敛「每日收盘数据包」为唯一日采入口，列出涨停池一源五写、指数三处存、僵尸字段、6 张表幂等缺口、采集规范 8 条与 P0~P2 分期建议。
- [新功能] 大盘情绪页阶段感知读数：`/overview` 按 A 股交易阶段自动决定来源——盘中/午休/收盘竞价附实时块（60s TTL 缓存，避免每次请求拉全市场）、盘后 15:05 起后台线程自动补采当日快照（按日幂等）、盘前/节假日纯读库零外部请求，`realtime=true` 保留显式强制实时；响应新增 `market_phase`，快照表新增 `sh_close`/`sh_chg_pct`（上证指数收盘点位/涨跌幅，旧库启动自动补列）；前端补全指数行情卡（盘中实时/收盘快照双来源标注）、趋势卡上证指数真实数值、盘中 60s 自动刷新（页面不可见时暂停）与页头阶段标签。
- [新功能] 大盘情绪页历史回填：`backfill_recent_snapshots` 一次性补采最近 N 个交易日快照（指数点位/涨跌走日线链路、沪市成交额缺失时经 tushare `index_daily` 补齐、三类池/梯队/炸板率按日期拉取落库；涨跌家数/外盘/情绪温度无历史免费源置空），快照齐全的日期不覆盖，数据源限流致量能缺失时重跑自动补齐。
- [改进] Web 端落地 UI 设计系统 v0.3 基础阶段：恢复标准 rem 字号基准（删除 html 14px 全局缩放）、默认主题切换为浅色（背景 #f0f4f9、边框 #e2e8f0、新增 `--primary-dark` token）、侧边栏导航改为实体 emoji 图标（功能控件保留 lucide）。
- [改进] Web 全站清扫 ≤10.5px 子像素字号至 11px 下限（22 个文件 108 处 + index.css），对齐 ui-layout design.md「<11px 全站禁止」硬规则；涨跌色统一 token（`--color-up`/`--color-down`）迁移待后续批次。
- [新功能] 同花顺数据源接入指数快照 `get_main_indices` 并作为 A 股指数优先源（`Manager.get_main_indices` cn 门面 THS 先行，失败回落 efinance/akshare/tickflow 链路）；大盘情绪收盘快照新增沪深成交额拆分 `sh_amount`/`sz_amount`（亿元，源自上证指数/深证综指成交额，自动建表）；主力净流入/北向/60 日新高新低仍留空（THS 无对应端点）。
- [新功能] 新增同花顺 Financial-API 数据源 `ThsFetcher`（P0+P1：A 股历史日 K fallback + 涨停/跌停/炸板池第二源 + meta 代码搜索辅助），配置 `HITHINK_FINANCE_API_KEY` 启用，留空完全停用不影响现有数据链路（方案见 `redesign/features/data-source/design.md`；单位口径/字段名联调待有 key 后按核对指南补验）。
...
- [新功能] 大盘情绪页聚焦周期化改造：新增财经日历数据源 `get_economic_calendar`（百度经济数据）与板块区间行情 `get_sector_history`（东财 push2his 直连 + 板块名→代码映射 + 重试），「明日重点·关键事件」改为明日财经日历（importance≥2、例行噪音过滤、按时间排序 ≤8 条），周聚焦事件改为未来 7 天滚动窗口每日采集刷新（守卫按当日已写日历事件判断）、月聚焦事件在本月首个采集日一次生成整月固定（假期顺延节后首个采集日补生成；周/月日历逐日前瞻 + 近 7 天/上月已落库快讯按关键词强度 Top5 作为「延续关注」回顾，`/focus` 响应新增 `review_events` 追加字段且前瞻仅返回日历事件），事件去重键补 scope 维度；周/月聚焦板块与个股改为上一周期回溯聚合（区间整周/整月涨幅榜 + 最高连板空间榜，写入焦点股/板块表周期一次），「明日重点」板块/个股数据源切换为涨停池明细（行业聚集度 + 连板高度），采集流程不再写每日焦点股/板块行。
- [改进] 大盘情绪页市场聚焦 UI 修缮（round6）：事件/板块/个股 tab 与周期切换同规格分段控件且占满整行；字阶整体上调（标题 15px、正文 13px、chip 最小 11px，清除 ≤10.5px），重点关注卡改实底+左侧红边+⭐徽章、榜单新增序号方块与 15px 主数字；周聚焦事件改为未来 7 天逐日展示（无日程日罗列占位）+ 星期导航锚点跳转，周期头新增生成语义徽章与前瞻/重点统计行；外国财经事件时间附当地换算（如「20:30 北京（美东 08:30）」，浏览器 Intl 含夏令时）。
- [改进] 大盘情绪页聚焦区三分 tab 重构：明日重点/周聚焦/月聚焦均提供 🔥事件/📊板块/⭐个股 三个内容 tab（此前月聚焦仅板块、明日重点无 tab），周/月事件按日期分组渲染并附上一周期回顾区，事件卡新增重点关注/全球关注/延续关注三级影响标签体系，未来周期无板块个股数据处以回溯提示条说明数据来源；五档情绪阶段胶囊中间档（低迷/弱势/中性/活跃/强势）底色与描边加深至与极端档同深度（/15 + ring/40），五档突出程度一致、仅以颜色区分情绪。
